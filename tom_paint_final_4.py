...
def undo_last_paint ( last_coordinate ):
  l_c .append([last_coordinate])
  #l_c += l_c
  last_element = len( l_c) - 1   # last element 
  if len(l_c[last_element]) > 0:
    for i in range (len(l_c[last_element])):
      #screen.set_at ((l_c[y][0], l_c[y][0]), BACKGROUND )
      pygame.draw.line(screen, BACKGROUND,  (l_c[last_element][i][0], l_c[last_element][i][1]),(l_c[last_element][i][2],l_c[last_element][i][3]), undo_size)
      
    undo_coordinate.clear() #	clear last element
...
#what_color_coord_center[0] = color_blue.cente
    ...
    # TEMPORARLY USED TO TEST UNDO AND REDO
...
#========= continious line===========				
  ...
  # COLOR VIEWER WILL also BE USED to UNDO last CHANGE  
    ...
    undo_color = paint_color
    undo_size = pen_size
    undo_coordinate.append([x, y , p, q])
