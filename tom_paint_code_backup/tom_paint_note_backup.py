...
def save_screen():
	typing = True
	enter_name = my_medium_font.render(" FILE NAME ", 1,(0,255,0))
		...
		for events in pygame.event.get():
			...
			if events.type == pygame.KEYDOWN :
				if events.key == pygame.K_RETURN :
					#pygame.image.save(screen, file_name_l)
					typing = False
				if events.key== pygame.K_BACKSPACE:
					player = player[: -1]	
				else:
						
					file_name += events.unicode
					file_name_l.append(events.unicode)
					
				file_name_txt= my_small_font.render(str(file_name), 1,(255,0, 0))		
				screen.blit(file_name_txt, (110, 160))
