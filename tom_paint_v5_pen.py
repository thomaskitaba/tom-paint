...
def save_screen():
  ...
  pygame.draw.rect(screen,BACKGROUND,pygame.Rect(0, screen_height -  color_pallet_size * 2.7 , screen_width, color_pallet_size * 2.7))
...
#
      ...
      if distance_with_blue <= color_pallet_size * 0.5:
        undo_index[0]  += 1
        l = len(undo_coordinate)
        
        for i in range(l - undo_index):
          
          undo_paint(undo_coordinate[i][0], undo_coordinate[i][1], undo_coordinate[i][2])
          #undo_paint(  i, 600 , RED)
...
#========= continious line===========				
    ...
    if pygame.mouse.get_pressed()[0] and pen_mode==  True and mousey <= screen_height - 230 and mousey >= ot +  color_pallet_size:
        ...
        save_pixel_data.append([mousex,mousey]) # to be used to UNDO pixel information
...
#====== CONTINIOUS LINE ========		
    if pygame.mouse.get_pressed()[0] and shape_mode_line_continious ==  True and mousey <= screen_height - 230 and mousey >= ot +  color_pallet_size:
        ...
        if s_o_p_c_cont > 2:
          ...
          #paper_rustle.play()			
    x = shape_coordinate_line_continious[0]	
    y = shape_coordinate_line_continious[1]	
    p = shape_coordinate_line_continious[2]
    q = shape_coordinate_line_continious[3]
    paint_continious_line(x,y,p,q, paint_color)
    
  #	paint_shapes(x,y,p,q, paint_color)
  #	paint_shapes(0, 100 , 500, 100, paint_color)
    #	if  s_o_p_c_cont > 2:	#paint_shapes(paint_coordinate_line_continious[s_o_p_c_cont - 2][0], paint_coordinate_line_continious[s_o_p_c_cont - 2][1], paint_coordinate_line_continious[s_o_p_c_cont - 1][0]  ,paint_coordinate_line_continious[s_o_p_c_cont - 1][1] , paint_color)
...
#==========CIRCLE===================
    if pygame.mouse.get_pressed()[0] and shape_mode_circle ==  True and mousey <= screen_height - 230 and mousey >= ot +  color_pallet_size:
      ...
      start_x= shape_coordinate_rectangle[0] 
      start_y=shape_coordinate_rectangle[1] 
      end_x= shape_coordinate_rectangle[2] 
      end_y=shape_coordinate_rectangle[3]
...
#==================================		
  ...
  # MOUSEUP FOR    UNDO	
    if pygame.mouse.get_pressed()[0]:
      mousex, mousey = pygame.mouse.get_pos()
      save_pixel_data.append([mousex, mousey])
      
    if event.type == pygame.MOUSEBUTTONUP :
      p = color_pallet_size +20
      q = int(screen_height - color_pallet_size * 2.7)
      l = len(save_pixel_data)
      
      if save_pixel_data[l - 1][1] > p and save_pixel_data[l -1][1] < q:
        
        for rows in range(p, q):
          
          for cols in range (screen_width):
            pixel_color = screen.get_at((rows,cols))
        
            undo_coordinates.append([rows, cols, RED])
            
    if event.type == pygame. MOUSEBUTTONUP and pen_mode== True:# and len(paint_coordinate_rectangle) > 1:
        paint_coordinate_pen.clear()
